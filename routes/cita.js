...
Router.get("/:IdCita", async (solicitud, respuesta) => {
  ...
  if (isValidToken) {
    ...
    try {
      ...
      console.error(
        `Error al buscar la cita ${solicitud.params.IdCita}:`,
        error
      );
      respuesta.status(500).json({ error: "Error interno del servidor" });
    }
...
Router.delete("/:IdCita", async (solicitud, respuesta) => {
  ...
  if (isValidToken) {
    ...
    try {
      ...
      console.error(
        `Error al borrar la cita ${solicitud.params.IdCita}:`,
        error
      );
      respuesta.status(500).json({ error: "Error interno del servidor" });
    }
...
Router.put("/:IdCita", async (solicitud, respuesta) => {
  ...
  if (isValidToken) {
    ...
    try {
      const resultado = await Cita.Actualizar(
        solicitud.params.IdCita,
        solicitud.body
      );
      respuesta.json({ Cita: resultado, Token: newToken });
    } catch (error) {
      console.error(
        `Error al actualizar la cita ${solicitud.params.IdCita}:`,
        error
      );
      respuesta.status(500).json({ error: "Error interno del servidor" });
    }
...
Router.get("/solicitante/:IdSolicitante", async (solicitud, respuesta) => {
  try {
    ...
    const citas = await Cita.BuscarPorSolicitante(
